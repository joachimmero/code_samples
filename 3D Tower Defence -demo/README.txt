...
These scripts are from a 3D tower defence demo. This demo was made as simple as possible to meant to test if a game
like it would be fun to play.
The game is supposed to be a casual party game, that is meant to be played with a group of people. In the game you 
...
- Modified Unity NavMesh -system, that allows enemies to aggro the plaeyr, towers and the hub.
- Two types of enemies. A minion (aggros only the hub) and a swordman (aggros the player, towers and hub).
- Player character who has a weapon, that knocks enemies back.
- Camera that adjusts itself depending on player's positions.


Scripts: (All the scripts have an "Offline"-tag, because I have also tested an online version of this demo.)
...
EnemyController
- Handles the enemies navigation, aggroing and initiating their attacks. Also hold functions for the enemies to 
...
ItemManager
- Hold the information of each players items. Currently only has a field for the player's resources.

Movement
- Handles the player's movement, initiating attacks and targeting enemies.
...
This demo was completely designed and programmed by myself. I also animated the player's and enemy's attack.

What I learnt:

Quick iterations of different design ideas is a great style of working.
